"""Streamlit web UI for Ollama Debate (web interface layer)."""
from datetime import date
from pathlib import Path

import streamlit as st

from arena import (
    Arena,
    BattleResult,
    build_markdown,
    build_participants,
    check_ollama_running,
    ensure_models_available,
    llm_options_from_config,
    load_config,
    save_debate_to_md,
    SpeechTurn,
    topic_to_slug,
)


DEBATES_DIR = "debates"

# Default Streamlit sidebar ≈21.25rem; +30% width
_SIDEBAR_WIDTH_REM = 21.25 * 1.3
# st.text_area height= is in pixels; prior default 80 → +100%
_TOPIC_TEXT_AREA_HEIGHT_PX = 160


def _inject_app_css() -> None:
    """Sidebar width plus light polish (Streamlit DOM varies by version)."""
    w = _SIDEBAR_WIDTH_REM
    st.markdown(
        f"""
        <style>
            .stApp {{
                font-feature-settings: "kern" 1, "liga" 1;
            }}
            section[data-testid="stSidebar"] {{
                width: {w}rem !important;
                min-width: {w}rem !important;
                border-right: 1px solid rgba(28, 25, 23, 0.08);
            }}
            section[data-testid="stSidebar"] > div {{
                width: 100% !important;
            }}
            section[data-testid="stSidebar"] .block-container {{
                padding-top: 1.25rem;
            }}
            [data-testid="stHeader"] {{
                background: rgba(250, 249, 246, 0.92);
                backdrop-filter: blur(8px);
            }}
        </style>
        """,
        unsafe_allow_html=True,
    )


def load_config_safe():
    """Load config for web UI; return None and report errors via Streamlit."""
    try:
        return load_config()
    except FileNotFoundError as e:
        st.error(str(e))
        return None
    except ValueError as e:
        st.error(str(e))
        return None
    except Exception as e:  # pragma: no cover - defensive
        st.error(f"Error loading config: {e}")
        return None


def ensure_ollama() -> bool:
    """Return True if Ollama is reachable; show st.error and return False otherwise."""
    try:
        check_ollama_running()
        return True
    except Exception as e:
        st.error(f"Ollama server is not running: {e}. Start Ollama or run `ollama serve`.")
        return False


def ensure_models(model_m: str, model_s: str, model_judge: str) -> bool:
    """Ensure models exist; pull if missing. Return True on success."""
    try:
        ensure_models_available(model_m, model_s, model_judge)
        return True
    except Exception as e:
        st.error(f"Model error: {e}")
        return False


def render_speech(entry: SpeechTurn, *, round_num: int, total_rounds: int) -> None:
    """Render one speech block in Streamlit."""
    name = entry.name
    icon = entry.icon
    think = (entry.think or "").strip()
    speech = entry.speech
    accent = "#6b2d7b" if name == "Machiavelli" else "#166a73"
    with st.container(border=True):
        st.markdown(
            f'<p style="margin:0 0 0.35rem 0;padding:0.2rem 0 0.35rem 0.6rem;border-left:4px solid {accent};'
            f'font-weight:600;font-size:1.1rem;line-height:1.35;">{icon} {name.upper()}</p>',
            unsafe_allow_html=True,
        )
        st.caption(f"Round **{round_num}** of **{total_rounds}**")
        if think:
            with st.expander("🔍 Thoughts", expanded=False):
                st.caption(think)
        st.markdown(speech)
        p = entry.prompt_tokens
        c = entry.completion_tokens
        st.caption(f"This reply — prompt **{p}**, completion **{c}**, subtotal **{p + c}**")
    st.divider()


def main():
    st.set_page_config(page_title="Ollama Debate", page_icon="🏛", layout="wide")
    _inject_app_css()
    st.title("🏛 Ollama Debate")
    st.caption("Historical court: Socrates vs Machiavelli, with a Judge. Powered by Ollama.")

    config = load_config_safe()
    if not config:
        st.stop()

    models_cfg = config.get("models") or {}
    settings_cfg = config.get("settings") or {}

    with st.sidebar:
        st.markdown("### Court settings")
        st.caption("Topic, rounds, and Ollama model names.")
        st.divider()
        st.subheader("Debate")
        topic = st.text_area(
            "Topic",
            label_visibility="collapsed",
            placeholder="What should they argue about?",
            value="What is better for society: total state control or complete anarchy?",
            height=_TOPIC_TEXT_AREA_HEIGHT_PX,
        )
        rounds = st.number_input("Rounds", min_value=1, max_value=10, value=settings_cfg.get("default_rounds", 2), step=1)
        st.subheader("Models")
        model_m = st.text_input("Machiavelli", value=models_cfg.get("machiavelli", "llama3:latest"))
        model_s = st.text_input("Socrates", value=models_cfg.get("socrates", "qwen2.5-coder:7b"))
        model_judge = st.text_input("Judge", value=models_cfg.get("judge", "llama3.2:latest"))
        st.divider()
        st.subheader("Run")
        stream_output = st.checkbox("Stream model output", value=False)
        run_clicked = st.button("Start debate", type="primary", use_container_width=True)
        st.caption("Ollama must be running. Missing models are pulled when needed.")

    if not run_clicked:
        st.info("Set the topic and models in the sidebar, then click **Start debate**.")
        st.stop()

    if not topic.strip():
        st.warning("Please enter a debate topic.")
        st.stop()

    if not ensure_ollama():
        st.stop()

    with st.spinner("Checking / pulling models..."):
        if not ensure_models(model_m, model_s, model_judge):
            st.stop()

    st.markdown("---")
    with st.container(border=True):
        st.markdown("##### Session overview")
        st.markdown(topic.strip() or "_(empty topic)_")
        st.caption(
            f"Rounds **{rounds}** · Machiavelli `{model_m}` · Socrates `{model_s}` · Judge `{model_judge}`"
        )
    if stream_output:
        st.caption("Streaming shows partial text as it arrives; each finished turn still appears as a full card below.")
    st.divider()

    llm_options = llm_options_from_config(config)

    machiavelli, socrates, judge = build_participants(
        config,
        model_m=model_m,
        model_s=model_s,
        model_judge=model_judge,
    )

    arena = Arena(machiavelli=machiavelli, socrates=socrates, judge=judge, llm_options=llm_options)

    n_rounds = int(rounds)
    total_llm_steps = n_rounds * 2 + 1
    speech_idx = 0

    # Transcript must NOT live inside st.status: status.update() can drop earlier
    # children when the label refreshes, which erased prior speeches during streaming.
    prog = st.progress(0, text="Preparing debate…")
    transcript = st.container()
    stream_slot: dict = {"ph": None}
    stream_accum = {"text": ""}

    def on_stream_begin(role: str) -> None:
        stream_accum["text"] = ""
        with transcript:
            stream_slot["ph"] = st.empty()
        if stream_slot["ph"] is not None:
            stream_slot["ph"].markdown(f"**{role}** is generating…")

    def on_stream_chunk(role: str, delta: str) -> None:
        stream_accum["text"] += delta
        ph = stream_slot["ph"]
        if ph is not None:
            ph.markdown(f"### {role}\n\n{stream_accum['text']}▍")

    with st.status("Debate in progress…", expanded=True) as run_status:
        def on_speech(entry: SpeechTurn) -> None:
            nonlocal speech_idx
            speech_idx += 1
            round_num = (speech_idx - 1) // 2 + 1
            run_status.update(
                label=f"Round {round_num}/{n_rounds} — {entry.name} replied",
                state="running",
            )
            prog.progress(
                min(speech_idx / total_llm_steps, 1.0),
                text=f"Round {round_num}/{n_rounds} · {entry.name}",
            )
            if stream_output and stream_slot["ph"] is not None:
                stream_slot["ph"].empty()
                stream_slot["ph"] = None
            with transcript:
                render_speech(entry, round_num=round_num, total_rounds=n_rounds)

        def on_verdict(text: str, p: int, c: int) -> None:
            nonlocal speech_idx
            speech_idx += 1
            prog.progress(1.0, text="Judge verdict")
            run_status.update(label="Judge finished", state="running")
            if stream_output and stream_slot["ph"] is not None:
                stream_slot["ph"].empty()
                stream_slot["ph"] = None
            with transcript:
                with st.container(border=True):
                    st.markdown(
                        '<p style="margin:0 0 0.35rem 0;padding:0.2rem 0 0.35rem 0.6rem;border-left:4px solid #b8860b;'
                        'font-weight:600;font-size:1.1rem;line-height:1.35;">⚖️ VERDICT</p>',
                        unsafe_allow_html=True,
                    )
                    st.markdown(f"**{text}**")
                    st.caption(
                        f"Judge call only — prompt **{p}**, completion **{c}**, subtotal **{p + c}** "
                        "(not the full session total)"
                    )

        result: BattleResult = arena.run_battle(
            topic.strip(),
            rounds=n_rounds,
            on_speech=on_speech,
            on_verdict=on_verdict,
            stream=stream_output,
            on_stream_begin=on_stream_begin if stream_output else None,
            on_stream_chunk=on_stream_chunk if stream_output else None,
        )

        run_status.update(label="Debate finished", state="complete")

    st.subheader("Token usage (session)")
    c1, c2, c3 = st.columns(3)
    c1.metric("Prompt", str(result.token_prompt))
    c2.metric("Completion", str(result.token_completion))
    c3.metric("Total", str(result.token_total))
    st.caption("Sum of every LLM call in this run (all rounds plus judge).")

    if result.interrupted:
        st.warning("Debate interrupted by user. Partial transcript above.")

    debates_dir = settings_cfg.get("debates_dir", DEBATES_DIR)
    token_stats = {
        "prompt": result.token_prompt,
        "completion": result.token_completion,
        "total": result.token_total,
    }
    md_content = build_markdown(
        topic=result.topic,
        model_m=result.machiavelli_model,
        model_s=result.socrates_model,
        model_judge=result.judge_model,
        transcript_entries=result.transcript_entries,
        verdict=result.verdict,
        token_stats=token_stats,
    )

    filepath_str: str | None = None
    try:
        filepath_str = save_debate_to_md(
            topic=result.topic,
            model_m=result.machiavelli_model,
            model_s=result.socrates_model,
            model_judge=result.judge_model,
            transcript_entries=result.transcript_entries,
            verdict=result.verdict,
            token_stats=token_stats,
            debates_dir=debates_dir,
        )
        st.success(f"Debate saved to **{filepath_str}**")
    except OSError as e:
        st.error(f"Could not save file: {e}")

    download_name = (
        Path(filepath_str).name
        if filepath_str
        else f"{date.today().isoformat()}_{topic_to_slug(result.topic)}.md"
    )
    st.download_button(
        "Download transcript (.md)",
        data=md_content.encode("utf-8"),
        file_name=download_name,
        mime="text/markdown",
        type="primary",
        use_container_width=True,
    )
    with st.expander("Markdown preview", expanded=False):
        st.code(md_content, language="markdown")


if __name__ == "__main__":
    main()
