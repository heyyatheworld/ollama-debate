...
# Import after potential sys.path fix so modules are loadable
...
def test_topic_to_slug_basic():
    ...
    assert arena.topic_to_slug("What is justice?") == "what_is_justice"
    assert arena.topic_to_slug("  State vs Anarchy  ") == "state_vs_anarchy"


def test_topic_to_slug_empty_and_fallback():
    """Empty or only-punctuation topic yields 'debate'."""
    assert arena.topic_to_slug("") == "debate"
    assert arena.topic_to_slug("???") == "debate"
...
def test_topic_to_slug_length_cap():
    ...
    long_topic = "a" * 300
    slug = arena.topic_to_slug(long_topic)
    assert len(slug) == 240
    assert slug == "a" * 240
...
def test_parse_args_cli_overrides_config():
    ...
    with patch.object(sys, "argv", ["cli.py", "--model_m", "cli-model", "--rounds", "3"]):
        args = parse_args(config)
